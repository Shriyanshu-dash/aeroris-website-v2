 ...
 * Renders floating particles that connect with lines when close to each other
...
function init() {
    ...
    for (let i = 0; i < numberOfParticles; i++) {
        ...
        let color = '#6366F1'; // Indigo-ish color
    ...
    for (let a = 0; a < particlesArray.length; a++) {
        for (let b = a; b < particlesArray.length; b++) {
            ...
            if (distance < (canvas.width / 7) * (canvas.height / 7)) {
                ...
                ctx.strokeStyle = 'rgba(99, 102, 241,' + opacityValue * 0.4 + ')'; // Line color with dynamic opacity
                ctx.lineWidth = 1;
                ctx.beginPath();
