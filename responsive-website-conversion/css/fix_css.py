...
# Identify cut point
# We are looking for the end of the valid .faq-cta-card block
# .faq-cta-card { ... margin-bottom: 40px; }
# } (media query)
...
if idx != -1:
    ...
    if brace1 != -1:
        brace2 = content.find("}", brace1 + 1)
        if brace2 != -1:
            good_content = content[:brace2+1]
        else:
             # Maybe the media query closing brace is missing or corrupted?
             # Let's take up to brace1 and add the second brace manually
             good_content = content[:brace1+1] + "\n}"
    else:
        print("Critical: Cannot find closing brace for faq-cta-card")
        exit(1)
else:
    print("Critical: Cannot find marker")
    exit(1)
...
with open(style_path, 'w', encoding='utf-8') as f:
    f.write(good_content + clean_css)

print("File rewritten with encoding normalization.")
